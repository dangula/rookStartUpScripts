...
case $1 in
	...
	[Ss][Tt][Aa][Rr][Tt])
        ...
        sudo docker save rookd-ci |gzip >/to-host/rookd-ci.tar
        sudo docker save rook-operator-ci |gzip >/to-host/rook-operator-ci.tar
        sudo docker save rook-client-ci |gzip >/to-host/rook-client-ci.tar
